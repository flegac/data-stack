...
@router.post("/heatmap")
async def get_heatmap(points: list[list[float]], values: list[float], width: int = 512, height: int = 512):
...
def generate_heatmap(points, values, width, height):
    ...
    interpolated_values = griddata(points, values, (grid_x, grid_y), method="nearest", fill_value=0.0)

    # Normaliser les valeurs interpolées pour qu'elles soient entre 0 et 255
    interpolated_values_normalized = (interpolated_values - interpolated_values.min()) / (interpolated_values.max() - interpolated_values.min()) * 255
